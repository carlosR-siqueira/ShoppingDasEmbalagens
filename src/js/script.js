...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Fechar a barra de busca ao clicar fora dela
    ...
    if (searchIcon) {
        searchIcon.addEventListener("click", function (event) {
            ...
            // Se a barra já estiver ativa e houver texto, faz a busca
            if (searchBarContainer.classList.contains('active') && searchInput.value.trim() !== "") {
                buscarProdutos();
                return;
            }
            
            // Alterna a classe 'active' na barra de busca
            searchBarContainer.classList.toggle('active');
            
            // Foca no input quando a barra é aberta
            if (searchBarContainer.classList.contains('active')) {
                // Pequeno atraso para garantir que a animação CSS seja aplicada
                setTimeout(() => {
                    searchInput.focus();
                }, 100);
            }
